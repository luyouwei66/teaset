...
export default class ToastView extends Overlay.View {
  ...
  renderIcon() {
    ...
    if (!React.isValidElement(icon)) {
      ...
      image = (
        <Image
          style={{width: sp(Theme.toastIconWidth), height: sp(Theme.toastIconHeight), tintColor: Theme.toastIconTintColor}}
          source={imageSource}
          />
      );
    } else {
  ...
  renderText() {
    ...
    if (typeof text === 'string' || typeof text === 'number') {
      text = (
        <Text style={{color: Theme.toastTextColor, fontSize: sz(Theme.toastFontSize)}}>{text}</Text>
      );
    }
    return text;
  }
